...
api.get("/api/streaming", async (req, res) => {
    ...
    const sseCli = new sse_1.SSEClient(req, res);
    console.log("New SSE Client:", `${sseCli.clientInfo.remoteAddress}:${sseCli.clientInfo.remotePort}`, sseCli.clientInfo.userAgent);
    let connOpen = true;
    sseCli.once("close", () => {
        connOpen = false;
    });
    ...
    while (connOpen) {
        ...
        sseCli.sendTypedEvent("ping", JSON.stringify({ timestamp: new Date() }), count);
        await new Promise((resolve) => setTimeout(resolve, 1000));
    }
    console.log("Close SSE Client:", `${sseCli.clientInfo.remoteAddress}:${sseCli.clientInfo.remotePort}`, sseCli.clientInfo.userAgent);
});
exports.default = api;
